...
//  Serves the Vite-built frontend from /frontend/dist in production
// ════════════════════════════════════════════════════════════════
...
async function checkAndAwardBadges(client, userId) {
  const [progRes, testsRes, userRes, badgeRes] = await Promise.all([
    client.query('SELECT status, COUNT(*) c FROM chapter_progress WHERE user_id=$1 GROUP BY status', [userId]),
    client.query('SELECT COUNT(*) total, COUNT(CASE WHEN score/NULLIF(max_score,0)*100>=100 THEN 1 END) perfect FROM test_results WHERE user_id=$1', [userId]),
    client.query('SELECT streak, level FROM users WHERE id=$1', [userId]),
    client.query('SELECT badge_id FROM user_badges WHERE user_id=$1', [userId]),
  ]);
  const earned  = new Set(badgeRes.rows.map(r => r.badge_id));
  const prog    = Object.fromEntries(progRes.rows.map(r => [r.status, parseInt(r.c)]));
  const masterRes = await client.query(`
    SELECT s.id FROM subjects s
    JOIN chapters ch ON ch.subject_id = s.id
    LEFT JOIN chapter_progress cp ON cp.chapter_id = ch.id AND cp.user_id = $1
    GROUP BY s.id
    HAVING COUNT(ch.id) = COUNT(CASE WHEN cp.status >= 2 THEN 1 END) AND COUNT(ch.id) > 0
  `, [userId]);
  const stats = {
    completed:       prog[2] || 0,
    revised:         prog[3] || 0,
    tests:           parseInt(testsRes.rows[0].total),
    perfectTests:    parseInt(testsRes.rows[0].perfect),
    streak:          parseInt(userRes.rows[0].streak),
    level:           parseInt(userRes.rows[0].level),
    subjectMastered: masterRes.rows.length,
  };
  const newBadges = [];
  for (const bd of BADGE_DEFS) {
    if (!earned.has(bd.id) && bd.check(stats)) {
      await client.query('INSERT INTO user_badges (user_id, badge_id) VALUES ($1,$2) ON CONFLICT DO NOTHING', [userId, bd.id]);
      newBadges.push(bd);
    }
  }
  return newBadges;
...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!name || !email || !password || !board || !classNum)
    return res.status(400).json({ error: 'Missing required fields' });
  if (!['cbse', 'icse', 'karnataka'].includes(board))
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO users (name, email, password_hash, board, class_num, stream, school_name)
       ...
       RETURNING id, name, email, board, class_num, stream, xp, level, streak`,
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (lastV === yest) streak++;
    else if (lastV !== today) streak = 1;
    await pool.query('UPDATE users SET streak=$1, last_visit=$2 WHERE id=$3', [streak, today, user.id]);

    const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET, { expiresIn: '30d' });
    const { password_hash, ...safeUser } = { ...user, streak };
...
app.get('/api/me', auth, async (req, res) => {
  const { rows } = await pool.query(
    `SELECT id, name, email, board, class_num, stream, school_name,
            xp, level, streak, last_visit, avatar_color, created_at
...
app.get('/api/syllabus', auth, async (req, res) => {
  ...
  const result = await Promise.all(subjects.rows.map(async s => {
    const { rows: chapters } = await pool.query(
      'SELECT id, name, ch_type, sort_order FROM chapters WHERE subject_id=$1 ORDER BY sort_order', [s.id]
    );
    return { ...s, chapters };
  }));
  res.json(result);
...
app.get('/api/progress', auth, async (req, res) => {
  const { rows } = await pool.query(
    'SELECT chapter_id, status, updated_at FROM chapter_progress WHERE user_id=$1', [req.user.id]
  );
  const map = {};
  rows.forEach(r => { map[r.chapter_id] = { status: r.status, updated_at: r.updated_at }; });
  res.json(map);
});
...
app.post('/api/progress', auth, async (req, res) => {
  ...
  try {
    ...
    const old = await client.query(
      'SELECT status FROM chapter_progress WHERE user_id=$1 AND chapter_id=$2', [req.user.id, chapterId]
    );
    const oldStatus = old.rows[0]?.status ?? 0;
    await client.query(
    ...
    if (status > oldStatus) {
      ...
      if (xpEarned > 0) {
        ...
        const { rows: [u] } = await client.query('SELECT xp FROM users WHERE id=$1', [req.user.id]);
        await client.query('UPDATE users SET level=$1 WHERE id=$2', [calcLevel(u.xp), req.user.id]);
      }
    }
...
app.get('/api/tests', auth, async (req, res) => {
  const { rows } = await pool.query(
    'SELECT * FROM test_results WHERE user_id=$1 ORDER BY test_date DESC, created_at DESC', [req.user.id]
  );
  res.json(rows);
});
...
app.get('/api/badges', auth, async (req, res) => {
  const { rows } = await pool.query(
    'SELECT badge_id, earned_at FROM user_badges WHERE user_id=$1 ORDER BY earned_at', [req.user.id]
  );
  const earnedMap = Object.fromEntries(rows.map(r => [r.badge_id, r.earned_at]));
  res.json(BADGE_DEFS.map(b => ({ ...b, earned: !!earnedMap[b.id], earned_at: earnedMap[b.id] || null })));
});
...
app.get('/api/stats', auth, async (req, res) => {
  ...
  const progMap    = Object.fromEntries(prog.rows.map(r => [r.status, parseInt(r.c)]));
  const nextLvlXp  = LEVEL_THRESHOLDS[Math.min(parseInt(user.rows[0].level), LEVEL_THRESHOLDS.length - 1)];
  res.json({
    ...user.rows[0],
    nextLvlXp,
    levelThresholds: LEVEL_THRESHOLDS,
    progress:   progMap,
    totalTests: parseInt(tests.rows[0].total),
    avgScore:   parseFloat(tests.rows[0].avg_score || 0).toFixed(1),
  });
});

// ══════════════════════════════════════════════════════════════════
//  HEALTH  &  ONE-TIME SETUP
// ══════════════════════════════════════════════════════════════════
...
app.post('/api/setup', async (req, res) => {
  ...
  res.json({ ok: true, msg: 'Schema applied' });
});
